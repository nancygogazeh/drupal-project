    ...
    $(window).scroll(function(){
      var wScroll = $(this).scrollTop();
        ...
        if ($('.center_block').length) {
          ...
          $('.cloth1').css({'transform': 'translate('+ -Math.abs(offset) +'px)'});
          $('.cloth2').css({'transform': 'translate('+Math.abs(offset)+'px)'});
        ...
        if ($('.bottom1').length) {
          ...
          if(wScroll > $('#footer').offset().top - $(window).height()){
            var offset = (Math.min(0, wScroll - $('#footer').offset().top +$(window).height() - 350)).toFixed();
          }
        });
